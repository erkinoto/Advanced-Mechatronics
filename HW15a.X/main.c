 ...
 * Author: Erkin 
 *
 * Created on June 14, 2019, 1:57 PM
 */
...
int main() {
    ...
    int j;
    for (j = 0; j < 240; j++){
        LCD_drawPixel(j, (a[j]>>5)+75, ILI9341_YELLOW);
    }

    int k;
    for (k = 0; k < 240; k++){
        LCD_drawPixel(k, (b[k]>>5)+125, ILI9341_YELLOW);
    ...
    int l;
    for (l = 0; l < 240; l++){
        LCD_drawPixel(l, (c[l]>>5) +175, ILI9341_YELLOW);
