...
////        gZ = (data[7] << 8) | data[6];        
        ...
        xCoord = ((int) aX/1000.0);
        yCoord = ((int) aY/1000.0);
        sprintf(message, "xCoord = %6d  ", xCoord);
        print2LCD(message, x, y+30, ILI9341_WHITE, ILI9341_PURPLE);
        sprintf(message, "xCoord = %6d  ", yCoord);
    ...
    switch (appData.state) {
        ...
        case APP_STATE_MOUSE_EMULATE:
            ...
            //HW12:
                ...
                if (inc == 1) {
                    appData.xCoordinate = (int8_t) xCoord; //dir_table[vector & 0x07];
                    appData.yCoordinate = (int8_t) yCoord; //dir_table[(vector + 2) & 0x07];
                    inc = 0;
                }
                else {
